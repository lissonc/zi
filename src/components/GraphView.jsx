...
// Force parameters — all forces are scaled by alpha so they naturally wind down
const REPULSION        = 1600
const REPULSION_CUT_SQ = 240 * 240  // skip pairs further apart than 240px
const SPRING_K         = 0.06
const SPRING_LEN       = 80
const CENTER_K         = 0.016
const VELOCITY_DECAY   = 0.38       // fraction of velocity removed each tick (higher = more friction)
const MAX_V            = 12         // hard velocity cap per tick

// Simulated annealing — alpha decays from 1 → ALPHA_MIN over ~300 ticks
const ALPHA_INIT  = 1.0
const ALPHA_DECAY = 0.0228          // 1 - (ALPHA_MIN/ALPHA_INIT)^(1/300)
...
/** Fibonacci (sunflower) spiral — distributes N points evenly over a disc. */
...
// ── Physics tick ─────────────────────────────────────────────────────────────
// sim = { nodes: [{id,x,y,vx,vy,pinned}], edges: [{source,target}], alpha }
// Mutates sim in place; returns new alpha.
...
function tick(sim) {
  ...
  // 1. Repulsion (Coulomb) with hard distance cutoff
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      const a = nodes[i], b = nodes[j]
      const dx = b.x - a.x, dy = b.y - a.y
      const dSq = dx * dx + dy * dy
      if (dSq > REPULSION_CUT_SQ) continue           // skip distant pairs
  ...
  // 3. Weak gravity toward canvas centre (prevents drift of isolated nodes)
  const cx = W / 2, cy = H / 2
  ...
  // 4. Integrate: apply friction, clamp speed, update position
  for (const n of nodes) {
...
export default function GraphView({ entries, onEdit }) {
  // physRef holds the mutable simulation state: { nodes, edges, alpha }
  ...
  // Pan / zoom — committed state + refs so event handlers avoid stale closures
  ...
  const dragRef  = useRef(null)   // {nodeId, prevCX, prevCY}
  ...
  const startSim = useCallback((resetAlpha = true) => {
    ...
    function animate() {
      ...
      // Sync positions to React state every 3 ticks (reduces re-renders)
  ...
  // ── Init / reinit when entries change ──────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const nodes = entries.map((e, i) => {
      ...
      // Preserve positions of nodes already in the graph
      if (prev) return { id: e.id, x: prev.x, y: prev.y, vx: 0, vy: 0, pinned: false }
      // Fibonacci spiral — far better initial distribution than a single circle
    ...
    startSim(false) // alpha already set above
    return () => { if (frameRef.current) cancelAnimationFrame(frameRef.current) }
  }, [entries, startSim])

  // ── Precompute per-entry radius (avoids O(n) scan per render) ──────────────

  const radiusMap = {}
  for (const e of entries) {
    const usedBy = entries.filter(x => x.componentIds.includes(e.id)).length
    radiusMap[e.id] = baseRadius(usedBy)
  }
  ...
  function onMouseUp() {
    if (dragRef.current) {
      ...
      startSim() // re-warm the simulation after a drag
    }
